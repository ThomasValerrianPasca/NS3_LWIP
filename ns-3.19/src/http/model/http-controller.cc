  ...
  if (CheckNextRequest (m_clientContainer))
    {
      ...
      stringstream ss;
     ss << "GET "<<"Client" << m_clientId << "Port80Size" << "123"<< ".html"<<" HTTP/1.1\r\n\r\n";
// ss << "GET \r\n\r\n";
    string s1=ss.str();
      uint8_t strbuffer[s1.length()];
      strcpy((char *)strbuffer,s1.c_str());
      ...
      while (bytesToSend)
        {
          ...
          if (bytesToSend > m_segmentSize)
            {
              bytesAccepted = socket->Send (Create<Packet> (strbuffer,m_segmentSize));
            }
          else
            {
              bytesAccepted = socket->Send (Create<Packet> (strbuffer,bytesToSend));
            }
          if (bytesAccepted > 0)
            {
...
//      m_clientContainer.clientSentTime = Simulator::Now ().GetSeconds ();
      m_clientContainer.clientSentTime = (Simulator::Now ().GetSeconds ())+10;
...
/////////////////////////
  uint8_t cpckt[bytesToSend];
  populateHTMLfile((char *)cpckt,bytesToSend);
  ...
  while (bytesToSend)
    {
      ...
      if (bytesToSend > m_segmentSize)
        {
          bytesAccepted = socket->Send (Create<Packet> (cpckt,m_segmentSize));
        }
      else
        {
          bytesAccepted = socket->Send (Create<Packet> (cpckt,bytesToSend));
        }
      if (bytesAccepted > 0)
        {
          bytesToSend -= bytesAccepted;
...
void HttpController::populateHTMLfile (char* ran_str,int size_res)
{
strcpy(ran_str,"HTTP/1.1 200 OK \r\n\r\n<html><head><meta http-equiv=\"Content-Type\" content=\"text/html; charset=UTF-8\"></head><body>Congratulations.  You've downloaded the file clientXPortYSizeZ.html which contains");
int i = 0;
int index = 182;
for (i = 0; i < size_res - 207; i++) 
{
ran_str[index++] = (char)(rand() % 256);
}
uint8_t endHtml[] = "</body></html>";
   std::strcat((char *)ran_str,(char *)endHtml);
