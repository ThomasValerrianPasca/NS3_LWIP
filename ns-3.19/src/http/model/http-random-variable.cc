...
double log_normal_form(double p)
{
     double ans=(0.399 * 0.7299 * pow(2.718,-(pow((log(p)/log(2.718) - 8.37),2)/3.758)))/p;
  return ans;
...
double exponential_form(double p)
{
      double p2=(-0.033)*p;
     double ans=0.033 * (pow(2.718,p2));
  return ans;
...
HttpTimeBtwnPagesRandomVariableImpl::HttpTimeBtwnPagesRandomVariableImpl ()
{
  ...
  //double x= frand();  
 // m_loc_b=exponential_form(x);  
  m_scale2_b = m_gamma.GetValue (SHAPE_SCALE2_B, 1 / RATE_SCALE2_B);
}

uint32_t HttpTimeBtwnPagesRandomVariableImpl::Func()
{

 double p=frand();
  double ans=exponential_form(p);
  return (uint32_t) ans;
}
