...
Ipv4InterfaceContainer Ipv4AddressHelper::assignaddress( NetDeviceContainer nc, Ipv4Address adr)
{
	NS_LOG_FUNCTION_NOARGS ();
	  Ipv4InterfaceContainer retval;
	for (uint32_t i = 0; i < nc.GetN (); ++i) {
		Ptr<NetDevice> device = nc.Get (0);
		Ptr<Node> node = device->GetNode ();
		NS_ASSERT_MSG (node, "Ipv4AddressHelper::Assign(): NetDevice is not not associated "
				"with any node -> fail");
		...
		Ipv4InterfaceAddress ipv4Addr = Ipv4InterfaceAddress (adr, m_mask);
		ipv4->AddAddress (interface, ipv4Addr);
		ipv4->SetMetric (interface, 1);
		ipv4->SetUp (interface);
		  retval.Add (ipv4, interface);
