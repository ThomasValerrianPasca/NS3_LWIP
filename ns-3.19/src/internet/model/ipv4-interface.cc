...
Ipv4Interface::Send (Ptr<Packet> p, Ipv4Address dest)
{
	...
	TcpHeader seqheader;
	p->PeekHeader(seqheader);
	uint32_t desport=seqheader.GetDestinationPort();
	uint16_t pid=ipv4Header.GetIdentification();
	p->AddHeader(ipv4Header);
	//std::cout << ipv4Header.GetSource().Get() << " " << ipv4Header.GetSource() << std::endl;
	if (DynamicCast<LoopbackNetDevice> (m_device))
	{
		//std::cout<<"LoopbackNetDeviceLoopbackNetDeviceLoopbackNetDevice"<<std::endl;
	...
	for (Ipv4InterfaceAddressListCI i = m_ifaddrs.begin (); i != m_ifaddrs.end (); ++i)
	{
		if (dest == (*i).GetLocal ())
		{
			...
			//std::cout<<"wrong entry in loop back"<<std::endl;
	...
	//	std::cout<<"Needs ARP" << std::endl;
		...
		if (found)
		{
		//	std::cout<<"Address Resolved Send."<<std::endl;
			NS_LOG_LOGIC ("Address Resolved.  Send.");
			//int pkt=rand();
			if(ipv4Header.GetSource().Get()>=117440512 &&ipv4Header.GetSource().Get()<=117440767) {
				std::cout << desport << std::endl;
			if(Ipv4Interface::mode==1|| Ipv4Interface::mode==4|| Ipv4Interface::mode==6) {
				std::cout<<"IF"<<std::endl;
			//	std::cout << "only lte or UL_lte-DL_wifi" << std::endl;
				//Sending packet to LTE NETDEVICE
				m_device->Send (p, hardwareDestination,Ipv4L3Protocol::PROT_NUMBER);
			}


			else if(Ipv4Interface::mode==2) {
				// only wifi
				std::cout << "only wifi" << std::endl;
				RouterLayer router;
				uint32_t i=0;
				for(i=0;i<m_device->GetNode()->GetNDevices();i++) {
					if(m_device->GetNode()->GetDevice(i)->virtualtypeid()=="ns3::WifiNetDevice") {
						break;
					}
				}
				//Sending packet to WIFI NETDEVICE
				if(i!=m_device->GetNode()->GetNDevices()) {
					router.senduplink(p,m_device->GetNode()->GetDevice(i),2048);
				}
				//Sending packet to LTE NETDEVICE
				else
				{
			...
			else if(Ipv4Interface::mode==3) {
				//std::cout << "ltewifi in send\n" <<std::endl;
				//lte+wifi

				//	  std::cout << "lte+wifi" << std::endl;
				// 	  std::cout<<"Packet no"<<pktno<<"source ip"<<ipv4Header.GetSource()<<"destination ip"<<ipv4Header.GetDestination()<<std::endl;
				//int pkt=rand();
				if(pid%2==0)
				{
					pktno++;
					// 	  std::cout << "send through lte interface "<<pktno << "broadcast"<<m_device->GetBroadcast ()<<std::endl;

					m_device->Send (p, hardwareDestination,Ipv4L3Protocol::PROT_NUMBER);
				}
				else {
					pktno++;
					//    	  std::cout << "send through wifi interface" <<"broadcast"<<m_device->GetBroadcast ()<< std::endl;
					RouterLayer router;
					uint32_t i=0;
					for(i=0;i<m_device->GetNode()->GetNDevices();i++) {
						if(m_device->GetNode()->GetDevice(i)->virtualtypeid()=="ns3::WifiNetDevice") {
			...
			} else if(Ipv4Interface::mode==5) {

				if(desport%2==0)
				{
					pktno++;
					std::cout << "lte "<< desport << std::endl;
					// 	  std::cout << "send through lte interface "<<pktno << "broadcast"<<m_device->GetBroadcast ()<<std::endl;

					m_device->Send (p, hardwareDestination,Ipv4L3Protocol::PROT_NUMBER);
				}
				else {
					std::cout << "wifi "<< desport << std::endl;
					pktno++;
					//    	  std::cout << "send through wifi interface" <<"broadcast"<<m_device->GetBroadcast ()<< std::endl;
					RouterLayer router;
					uint32_t i=0;
	...
	{
		// std::cout<<"Doesn't need ARP"<<std::endl;
		NS_LOG_LOGIC ("Doesn't need ARP");
		if(ipv4Header.GetSource().Get()>=117440512 &&ipv4Header.GetSource().Get()<=117440767) {

		if(Ipv4Interface::mode==1 || Ipv4Interface::mode==4|| Ipv4Interface::mode==6) {
			// only lte or UL_lte-DL_wifi
		//	std::cout << "only lte or UL_lte-DL_wifi" << std::endl;
			//std::cout << "only lte Broadcast" <<Simulator::Now()<< std::endl;
		//	std::cout<<"IF1"<<std::endl;
			...
			if(i<m_device->GetNode()->GetNDevices()) {
				//std::cout<<"Sending through @@@@@"<<m_device->GetNode()->GetDevice(i)->virtualtypeid()<<std::endl;
				router.senduplink(p,m_device->GetNode()->GetDevice(i),2048);
			}
			else
			{
				//if(i<=m_device->GetNode()->GetNDevices())
				//std::cout<<"Sending local "<<std::endl;
				m_device->Send(p,m_device->GetBroadcast (),Ipv4L3Protocol::PROT_NUMBER);
			}
		}
			...
			if(pid%2==0)
			{
				//pktno++;
				//  std::cout << "send through lte interface "<<pktno << "broadcast"<<m_device->GetBroadcast ()<<std::endl;
				//  std::cout<<"destport no"<<desport<<"source ip"<<ipv4Header.GetSource()<<"destination ip"<<ipv4Header.GetDestination()<<std::endl;
				m_device->Send (p, m_device->GetBroadcast (),Ipv4L3Protocol::PROT_NUMBER);
			}
			else {
				//pktno++;
