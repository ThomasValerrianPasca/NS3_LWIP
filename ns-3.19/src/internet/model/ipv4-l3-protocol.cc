                          ...
                          const Address &to, NetDevice::PacketType packetType)
{
  ...
  NS_LOG_LOGIC ("Packet from " << from << " received on node " << 
                m_node->GetId ());
//std::cout<<"Packet from " << from << " received on node " << m_node->GetId ()<<std::endl;
                      ...
                      Ptr<Ipv4Route> route)
{
  ...
  //std::cout << "netdevice " << GetInterface (0)->GetDevice ()->GetTypeId() << std::endl; 
  Ipv4Header ipHeader;
  ...
  if (destination.IsBroadcast () || destination.IsLocalMulticast ())
    {
      ...
      for (Ipv4InterfaceList::iterator ifaceIter = m_interfaces.begin ();
           ifaceIter != m_interfaces.end (); ifaceIter++, ifaceIndex++)
        {
          ...
          // std::cout << outInterface->GetDevice()->virtualtypeid();
      ...
      for (uint32_t j = 0; j < GetNAddresses (ifaceIndex); j++)
        {
          ...
          if (destination.IsSubnetDirectedBroadcast (ifAddr.GetMask ()) && 
              destination.CombineMask (ifAddr.GetMask ()) == ifAddr.GetLocal ().CombineMask (ifAddr.GetMask ())   )
            {
              ...
              packetCopy->AddHeader (ipHeader);
              m_txTrace (packetCopy, m_node->GetObject<Ipv4> (), ifaceIndex);
            //   std::cout << outInterface->GetDevice()->virtualtypeid() << std::endl;
              outInterface->Send (packetCopy, destination);
              return;
            }
        }
                             ...
                             Ipv4Header const &ipHeader)
{
  ...
  //std::cout << "Send via NetDevice ifIndex " << outDev->GetIfIndex () << " ipv4InterfaceIndex " << interface << outInterface->GetDevice()->virtualtypeid() << std::endl;
