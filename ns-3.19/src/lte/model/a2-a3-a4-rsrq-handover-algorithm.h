...
#ifndef A2_A3_A4_RSRQ_HANDOVER_ALGORITHM_H
#define A2_A3_A4_RSRQ_HANDOVER_ALGORITHM_H
 ...
 */
class A2A3A4RsrqHandoverAlgorithm : public LteHandoverAlgorithm
{
public:
  /**
   * \brief Creates an A2-A4-RSRQ handover algorithm instance.
   */
  A2A3A4RsrqHandoverAlgorithm ();

  virtual ~A2A3A4RsrqHandoverAlgorithm ();
  ...
  friend class MemberLteHandoverManagementSapProvider<A2A3A4RsrqHandoverAlgorithm>;
...
private:
  // Internal methods
  void EvaluateHandover (uint16_t rnti, LteRrcSap::MeasResults measResults);
  bool IsValidNeighbour (uint16_t cellId);
  void UpdateNeighbourMeasurements (uint16_t rnti, uint16_t cellId,
                                    uint8_t rsrq);
 // void Runa3part (uint16_t rnti,LteRrcSap::MeasResults measResults);
  ...
  uint8_t m_measId;
    uint8_t N;
  LteRrcSap::MeasResults measResults1;
