...
LteEnbMac::GetTypeId (void)
{
  static TypeId tid = TypeId ("ns3::LteEnbMac")
    ...
    .AddAttribute ("TTL",
                    "count",
                    UintegerValue(0),
                    MakeUintegerAccessor (&LteEnbMac::TTL),
                    MakeUintegerChecker<uint32_t> (0, 10000))
...
LteEnbMac::DoSubframeIndication (uint32_t frameNo, uint32_t subframeNo)
{
  ...
  NS_LOG_DEBUG("downlink start");
  m_schedSapProvider->SchedDlTriggerReq (dlparams);
  NS_LOG_DEBUG("downlink stop");
  ...
  NS_LOG_DEBUG("uplink start");
  m_schedSapProvider->SchedUlTriggerReq (ulparams);
  NS_LOG_DEBUG("uplink stop");
}
  ...
  for (  uint32_t i  = 0; i < ind.m_dciList.size (); i++ )
    {
      ...
      //std::cout<<"FRAMENO = "<<m_frameNo<<" S_FRAMENO = "<< m_subframeNo<<" RNTI = " <<ind.m_dciList.at (i).m_rnti<< " MCS= "<<ind.m_dciList.at (i).m_mcs<<"Transmission block size= "<< ind.m_dciList.at (i).m_tbSize<<"\n";
    }
