...
RrFfMacScheduler::GetTypeId (void)
{
  static TypeId tid = TypeId ("ns3::RrFfMacScheduler")
    ...
    .AddAttribute ("TTL",
                    "count",
                    UintegerValue(0),
                    MakeUintegerAccessor (&RrFfMacScheduler::TTL),
...
RrFfMacScheduler::DoSchedDlTriggerReq (const struct FfMacSchedSapProvider::SchedDlTriggerReqParameters& params)
{
  ...
  NS_LOG_DEBUG("downlink start TTL = " << TTL);
  TTL=TTL+1;
  ...
  if (rbgAllocatedNum == rbgNum)
    ...
    NS_LOG_DEBUG("downlink stop TTL = " << TTL);
      return;
    }
  ...
  do
    {
      ...
      uint32_t rbgMask = 0;
      uint16_t i = 0;
     //std::cout<<" DL - Allocate user " << newEl.m_rnti << " LCs " << (uint16_t)(*itLcRnti).second << " bytes " << tbSize << " mcs " << (uint16_t) newDci.m_mcs.at (0) << " harqId " << (uint16_t)newDci.m_harqProcess <<  " layers " << nLayer<<std::endl;
...
RrFfMacScheduler::DoSchedUlTriggerReq (const struct FfMacSchedSapProvider::SchedUlTriggerReqParameters& params)
{
  ...
  NS_LOG_DEBUG("uplink start TTL = " << TTL);
  TTL=TTL+1;
  RefreshUlCqiMaps ();
  tti_temp++;

//std::cout<<"=============================TTI="<<tti_temp<<"================"<<std::endl;
//std::cout<<"Number of RB="<<(uint32_t)m_cschedCellConfig.m_ulBandwidth<<std::endl;
  // Generate RBs map
  ...
  for (it = m_ceBsrRxed.begin (); it != m_ceBsrRxed.end (); it++)
    {
     ...
     // std::cout << " UE " << (*it).first << " queue " << (*it).second << std::endl;
      NS_LOG_INFO (this << " UE " << (*it).first << " queue " << (*it).second);
    //  std::cout<<" Uplink UE " << (*it).first << " queue " << (*it).second<<std::endl;
  ...
  if (nflows == 0)
    ...
    NS_LOG_DEBUG("uplink stop TTL = " << TTL);
...
// std::cout << " NFlows " << nflows << " RB per Flow " << rbPerFlow << std::endl;
  ...
  do
    {
     ...
     // std::cout <<" RB Allocated " << rbAllocated << " rbPerFlow " << rbPerFlow << " flows " << nflows << std::endl;
...
//   /   std::cout << " RB Allocated " << rbAllocated << " rbPerFlow " << rbPerFlow << " flows " << nflows<<std::endl;
     ...
     // std::cout << " UL Allocation - UE " << (*it).first << " startPRB " << (uint32_t)uldci.m_rbStart << " nPRB " << (uint32_t)uldci.m_rbLen << " CQI " << cqi << " MCS " << (uint32_t)uldci.m_mcs << " TBsize " << uldci.m_tbSize << " harqId " << (uint16_t)harqId<< std::endl;
