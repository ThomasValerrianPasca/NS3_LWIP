...
std::string WifiNetDevice::virtualtypeid(void) {
    return "ns3::WifiNetDevice";
}
...
{
 // std::cout<<"Packet sent "<<"from  "<<from<<"to  "<<to<<std::endl;
  LlcSnapHeader llc;
  packet->RemoveHeader (llc);
  RouterLayer router;
// std::cout << router.wifi_Ap_macaddr << " "<< from << std::endl;
  if(router.wifi_Ap_macaddr==from) {
	//  std::cout << "got the downlink" << std::endl;
  ...
  if (!m_promiscRx.IsNull ())
    {
	 ...
	 // std::cout << "reached wifinetdevice forwardup" << std::endl;
	   router.forwardtolteepcenb(packet,1);
  }
}
